...
public interface RabbitMQRepository {
     ...
     *
     * @param emailGreetingsDto The EmailDto object containing the email address to send the message to.
     * @return ResponseEntity containing the response from the RabbitMQ service.
     */
    @PostMapping("/message/greetings")
    ResponseEntity<String> sendGreetingsMessage(@RequestBody EmailGreetingsDto emailGreetingsDto);
}
